...
function JobOffers() {
  ...
  const addJobOffer = async (jobOffer) => {
    const formattedJobOffer = {
      ...
      employerId: userId, // Include employerId when adding a new job offer
    };

    try {
      if (editIndex !== null) {
        await axios.put(`http://localhost:4000/api/jobs/${jobOffers[editIndex]._id}`, formattedJobOffer);
        jobOffers[editIndex] = formattedJobOffer;
        displayToast('Job offer updated successfully.', 'success');
      } else {
        const response = await axios.post('http://localhost:4000/api/jobs/create-job', formattedJobOffer);
        jobOffers.push(response.data);
        displayToast('Job offer added successfully.', 'success');
      }
  ...
  const removeJobOffer = async (index) => {
    ...
    try {
      await axios.delete(`http://localhost:4000/api/jobs/${job._id}?userId=${userId}`);
